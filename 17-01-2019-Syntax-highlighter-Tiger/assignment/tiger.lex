...
"\""(\\{escapes} | [^\\"])*"\""         => (current_pos := yypos - !pos_last_line ; Tokens.STRING (!current_pos , !lineNum) yytext );
{newline}                               => (lineNum := !lineNum+1; linePos := yypos :: !linePos; pos_last_line := yypos ;Tokens.NEWLINE (!current_pos , !lineNum) yytext);
